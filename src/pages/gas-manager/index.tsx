  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Popconfirm title="确定删除该地址吗？" onConfirm={() => handleDelete(record.key)} okText="删除" cancelText="取消">
          <Button type="link" size="small" className="text-yellow-500">删除</Button>
        </Popconfirm>
      ),
    },
  ];
      ...
      <div className="max-w-[1000px] mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-10 md:p-12 border border-[#e0e0e0] mt-[0px]">
          ...
          <div className="flex justify-center mb-4">
            ...
            <span className="font-mono text-blue-700 bg-blue-50 rounded px-2 py-1 text-base">{address}</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6 md:gap-0">
            <div className="flex flex-col items-center bg-blue-50 rounded-xl px-8 py-6 shadow flex-1 mx-2 min-w-[220px]">
              <div className="text-4xl font-extrabold text-blue-600 mb-1">{addressList.length}</div>
              <div className="text-gray-600 mb-2">绑定地址数量</div>
              <button className="px-6 py-2 rounded-lg font-bold shadow transition" onClick={() => setManageAddressOpen(true)}>绑定地址</button>
            </div>
            <div className="flex flex-col items-center bg-gray-50 rounded-xl px-8 py-6 shadow flex-1 mx-2 min-w-[220px]">
              <div className="text-4xl font-extrabold mb-1">{gasPrice}</div>
              <div className="text-gray-600 mb-2">Gas Price</div>
              <button onClick={handleEdit} className="px-6 py-2 rounded-lg font-bold shadow transition">编辑</button>
            </div>
            <div className="flex flex-col items-center bg-yellow-50 rounded-xl px-8 py-6 shadow flex-1 mx-2 min-w-[220px]">
              <div className="text-4xl font-extrabold text-yellow-500 mb-1">{balance} <span className="text-base font-normal">BNB</span></div>
              <div className="text-gray-600 mb-2">账户余额</div>
              <div className="flex gap-2">
                <button className="px-6 py-2 rounded-lg font-bold shadow transition" onClick={() => setRechargeOpen(true)}>充值</button>
                <button className="px-6 py-2 rounded-lg font-bold shadow transition" onClick={() => {/* TODO: 提现逻辑 */}}>提现</button>
              </div>
            </div>
          </div>
