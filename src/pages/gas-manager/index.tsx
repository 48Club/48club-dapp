...
export default function GasManager() {
  ...
  const sponsorRecordColumns = [
    {
      ...
      key: 'gas_cost',
      align: 'left' as const,
      render: (text: string) => <span className="font-medium text-green-600">{+text / 1e18} bnb</span>,
    },
    {
      title: <><span>service cost</span><a className='underline decoration-1 ml-[5px]' href="https://docs.48.club/48-soul-point">info</a></>,
    ...
    },
    {
      ...
      align: 'left' as const,
      width: '150px',
      render: (text: string) => <span className="font-mono text-sm text-blue-600 break-all">{text}</span>,
    },
    {
      title: '交易哈希',
      dataIndex: 'tx_hash',
  ...
  const [mainTab, setMainTab] = useState<'address' | 'recharge' | 'consume' | 'withdraw' | 'deposit' | 'withdraw'>(
    'address'
  )
  ...
  const handleDelete = async (addresses: any[]) => {
    ...
    const msg = `i authorize master account ${account?.toLowerCase()} to stop paying gas fees for sub-accounts ${addressLow.join(
      ','
  ...
  const columns = [
    {
      ...
      align: 'left' as const,
      render: (text: string) => (
        <span className="font-mono text-gray-700">{new Date(Number(text) * 1000).toLocaleString()}</span>
      ),
    },
    {
      title: '地址',
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Popconfirm
          ...
          disabled={!account}
        >
          <Button type="link" size="small" className="text-yellow-500" disabled={!account}>
            删除
          </Button>
        </Popconfirm>
      ),
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-10 md:p-12 border border-[#e0e0e0] mt-[0px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            <div className="flex flex-col items-center bg-blue-50 rounded-xl px-6 py-6 shadow">
              ...
              <Button
                className="px-4 py-1 rounded-lg font-bold shadow transition text-sm"
                onClick={() => setManageAddressOpen(true)}
                disabled={!account}
              >
                绑定地址
              </Button>
            ...
            <div className="flex flex-col items-center bg-green-50 rounded-xl px-6 py-6 shadow">
              ...
              <div className="flex gap-2">
                <Button
                  className="px-3 py-1 rounded-lg font-bold shadow transition text-xs"
          ...
          <div className="bg-white rounded-xl shadow border border-[#e0e0e0] p-6">
            <Tabs
              activeKey={mainTab}
              onChange={(key) =>
                setMainTab(key as 'address' | 'recharge' | 'consume' | 'withdraw' | 'deposit' | 'withdraw')
              }
            >
