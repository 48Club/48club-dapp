...
const Row: React.FC<{
    ...
    const ItemNode = (<>
        <div className="flex-1 md:hidden text-[12px] text-[#E2B201] font-[400] leading-[24px]">
            {data.tick}  <span className="ml-[8px] px-[6px] h-[17px] leading-[17px] font-[400] bg-[rgba(217,217,217,.4)] text-[10px] rounded-full text-[#1E1E1E]">{data.protocol}</span>
        ...
        <div className="flex-1 md:flex hidden text-[14px] text-[#E2B201] items-center font-[400] leading-[24px]">
            {data.tick} <span className="text-[12px]">({shorten(data.tick_hash)})</span> <span className="ml-[8px] px-[6px] h-[17px] leading-[17px] font-[400] bg-[rgba(217,217,217,.4)] text-[10px] rounded-full text-[#1E1E1E]">{data.protocol}</span>
        </div>
...
const SearchRow:React.FC<{
    ...
    return <div onClick={(e) => e.stopPropagation()} className="h-[51px] flex px-[12px] items-center cursor-default">
        <div className="flex-1 text-[#E2B201] font-[400] leading-[24px]">
            {data.tick} <span className="ml-[8px] px-[6px] py-[4px] bg-[rgba(217,217,217,.4)] text-[10px] rounded-full text-[#1E1E1E]">{data.protocol}</span>
        </div>
        {
...
const SelectedToken = () => {
    ...
    const { result, loading } = useInscriptionsSearchState()
    ...
    return <div className="w-full shadow pt-[32px] pb-[32px] min-h-[600px] px-[12px]">
        ...
        >
            <Input
                placeholder="Enter"
                className=" w-full rounded-[4px] h-full no-border bg-[#F9F9F9]"
                onChange={(val) => {
                    setSearchText(val.target.value)
                }}
                value={searchText}
                allowClear={{
                    clearIcon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex flex-row justify-between px-[20px] items-center leading-[24px] mb-[16px] text-[16px] font-[400]">
            <div className="flex-1 text-gray">Token</div>
            <div className="flex-1 text-gray justify-center flex">Balance</div>
