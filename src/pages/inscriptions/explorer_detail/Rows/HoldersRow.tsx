...
const HoldersRow: React.FC<{
    ...
    return (
        <div onClick={() => window.open(`${window.location.origin}/inscriptions/account?address=${data.address}`)} className="cursor-pointer hover:bg-[#f4f4f4] py-4 flex flex-row justify-between items-center border-t border-gray text-[14px]">
            <div className="w-[140px] text-[14px] font-[400] leading-[20px]">
                {data.rank}
            </div>
            <div className="w-[240px] flex items-center">
                <div className="flex items-center py-[20px] hover:opacity-50 cursor-pointer" onClick={(e) => {
                    e.stopPropagation()
